define(['d3'], function () {
    ...
    cy = function (commit, view) {
        var parent = view.getCommit(commit.parent),
            ...
            baseLine = view.baseLine,
            shift = view.commitRadius * 4.5,
            branches = [], // contar os branches existentes
            branchIndex = 0;
    ...
    // calcula o ponto x1 para linhas de ponteiro de commit
    ...
    // calcula o ponto y1 para linhas de ponteiro de commit
    ...
    HistoryView.prototype = {
        /**
         * @method getCommit
         * @param ref {String} o id ou um nome de tag que se refere ao commit
         * @return {Object} o objeto de datum de commit
         */
         ...
         * @method getCircle
         * @param ref {String} o id ou um nome de tag que se refere ao commit
         * @return {d3 Selection} o círculo SVG selecionado do d3
         */
        ...
        /**
         * @method render
         * @param container {String} seletor para o contêiner para onde renderizar o SVG
         */
        ...
        renderCommits: function () {
            ...
            this._calculatePositionData(); // faça isso duas vezes para se assegurar
            this._renderCircles();
        ...
        _parseTagData: function () {
            ...
            // descubra quais commits estãos sem branches
        ...
        _markBranchlessCommits: function () {
            ...
            // primeiro marque cada commit como sem branch
            ...
            for (b = 0; b < this.branches.length; b++) {
                ...
                if (branch.indexOf('/') === -1) {
                    ...
                    // apenas no caso de este ser um merge commit
                    while (parent2) {
         ...
         * @param ref1
         * @param ref2
         * @return {Boolean} se ref1 é ou não um ancestral de ref2
         */
        ...
        commit: function (commit, message) {
            ...
            if (!commit.parent) {
                if (!this.currentBranch) {
                    throw new Error('Não é uma boa ideia fazer commits em um estado HEAD separado.');
                }
        ...
        branch: function (name) {
            if (!name || name.trim() === '') {
                throw new Error('Você precisa dar um nome ao branch.');
            }

            if (name === 'HEAD') {
                throw new Error('Você não pode chamar seu branch de "HEAD".');
            }

            if (name.indexOf(' ') > -1) {
                throw new Error('Nomes de branch não podem conter espaços.');
            }
        ...
        deleteBranch: function (name) {
            ...
            if (name === this.currentBranch) {
                throw new Error('Não pode excluir o branch que atualmente está em check-out.');
            }

            branchIndex = this.branches.indexOf(name);

            if (branchIndex === -1) {
                throw new Error('Esse branch não existe.');
            }
        ...
        checkout: function (ref) {
            ...
            if (!commit) {
                throw new Error('Não foi possível encontrar o commit: ' + ref);
            }
        ...
        reset: function (ref) {
            ...
            if (!commit) {
                throw new Error('Não foi possível encontrar a referência: ' + ref);
            }
        ...
        revert: function (ref) {
            ...
            if (this.isAncestor(commit, 'HEAD')) {
                ...
                throw new Error(ref + 'não é um ancestral do HEAD.');
            }
        },
        ...
        merge: function (ref, noFF) {
            ...
            if (currentCommit.id === mergeTarget.id) {
                throw new Error('Já está atualizado.');
